        ...
        return [
            Stat::make('Active services', $activeServices)
                ->description($erroredServices > 0
                    ? $erroredServices.' failed to provision'
                    : 'None in error')
            ...
            Stat::make('Connected now', "{$connectedNow} / {$totalUsers}")
                ->description($totalUsers === 0
                    ...
                    : 'Of the users with an active config'),

            Stat::make('Bandwidth today', $this->formatBytes($down + $up))
                ->description($this->formatBytes($down).' down / '.$this->formatBytes($up).' up'),
        ];
    }
